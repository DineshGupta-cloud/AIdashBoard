...
def nearest_row(rows, spot):
    rows = [r for r in rows if r.get("strikePrice", 0) > 0]
    return min(rows, key=lambda x: abs(x["strikePrice"] - spot))
...
def main():
    ...
    files = {}
    for path in glob(os.path.join(STOCK_FOLDER, "*.json")) + glob(os.path.join(STOCK_FOLDER, "*.JSON")):
    ...
    confidence = min(abs(total_score) * 1.8, 95)

    # Print
    print("\n==============================")
    print("   BANKNIFTY FULL AI ENGINE")
    print("==============================")
    print("Spot Price   :", bn["spot"])
    print("Signal       :", final_signal)
    print("Confidence   :", round(confidence, 2), "%")
    print("PCR          :", bn["pcr"])
    print("Support      :", bn["support"])
    print("Resistance   :", bn["resistance"])
    print("Total Score  :", round(total_score, 2))

    print("\n------ STOCK REPORT ------")
    for r in sorted(report, key=lambda x: x["weight"], reverse=True):
        print(
            f'{r["symbol"]:12} | '
            f'Wt: {r["weight"]:5} | '
            f'{r["signal"]:8} | '
